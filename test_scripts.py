...
# ===========================================================================
# check_devbox_status tests (T030)
# ===========================================================================

from conftest import FakeAccessToken
from orchestrator_devbox import DevBoxInfo, DevBoxManager


@pytest.fixture
def mock_devbox_manager():
    """A DevBoxManager with mocked credential (no real Azure calls)."""
    cred = MagicMock()
    cred.get_token.return_value = FakeAccessToken()
    mgr = DevBoxManager(
        devcenter_endpoint="https://fake-dc.devcenter.azure.com",
...
@pytest.fixture
def devbox_env_vars(monkeypatch):
    """Set required environment variables for the CLI script."""
    monkeypatch.setenv("DEVCENTER_ENDPOINT", "https://fake-dc.devcenter.azure.com")
    monkeypatch.setenv("DEVBOX_PROJECT", "test-project")
    monkeypatch.setenv("DEVBOX_POOL", "test-pool")


class TestCheckSingleDevbox:
    """Tests for check_single_devbox()."""

    def test_returns_status_on_success(self, mock_devbox_manager):
        from check_devbox_status import check_single_devbox

        mock_devbox_manager.get_devbox_status = MagicMock(return_value=DevBoxInfo(
            name="shraga-box-01",
        ...
        result = check_single_devbox(mock_devbox_manager, "aad-user-123", "shraga-box-01")
    ...
    def test_returns_error_on_not_found(self, mock_devbox_manager):
        from check_devbox_status import check_single_devbox

        mock_devbox_manager.get_devbox_status = MagicMock(
            side_effect=Exception("Failed to get Dev Box status: 404 Not Found")
        )

        result = check_single_devbox(mock_devbox_manager, "aad-user-123", "nonexistent-box")

        assert "error" in result
        assert "not found" in result["error"].lower()

    def test_returns_error_on_generic_failure(self, mock_devbox_manager):
        from check_devbox_status import check_single_devbox
...
class TestListUserDevboxes:
    """Tests for list_user_devboxes()."""

    def test_returns_list_on_success(self, mock_devbox_manager):
        from check_devbox_status import list_user_devboxes

        mock_devbox_manager.list_devboxes = MagicMock(return_value=[
            {"name": "shraga-box-01", "provisioningState": "Succeeded", "powerState": "Running"},
            {"name": "shraga-box-02", "provisioningState": "Creating", "powerState": "Unknown"},
        ])

        result = list_user_devboxes(mock_devbox_manager, "aad-user-123")

        assert "error" not in result
        assert result["count"] == 2
        assert len(result["devboxes"]) == 2

    def test_returns_empty_list_when_no_boxes(self, mock_devbox_manager):
        from check_devbox_status import list_user_devboxes
    ...
    def test_returns_error_on_failure(self, mock_devbox_manager):
        from check_devbox_status import list_user_devboxes

        mock_devbox_manager.list_devboxes = MagicMock(
            side_effect=Exception("Failed to list Dev Boxes: 404 Not Found")
        )
...
class TestCheckDevboxStatusCli:
    """Integration-style tests for the CLI entry point (main function)."""

    def test_check_devbox_status_cli_by_name(self, devbox_env_vars, capsys):
        from check_devbox_status import main as cds_main

        fake_info = DevBoxInfo(
            name="shraga-box-01", user_id="aad-user-abc",
            status="Running",
            connection_url="https://devbox.microsoft.com/connect?devbox=shraga-box-01",
            provisioning_state="Succeeded",
        )

        with patch("check_devbox_status.DevBoxManager") as MockDBM:
            mock_mgr = MagicMock()
            mock_mgr.get_devbox_status.return_value = fake_info
            MockDBM.return_value = mock_mgr

            exit_code = cds_main(["--name", "shraga-box-01", "--user", "aad-user-abc"])

        assert exit_code == 0
        captured = capsys.readouterr()
        output = json.loads(captured.out)
        assert output["name"] == "shraga-box-01"
        assert output["provisioning_state"] == "Succeeded"

    def test_check_devbox_status_cli_list_user(self, devbox_env_vars, capsys):
        from check_devbox_status import main as cds_main

        with patch("check_devbox_status.DevBoxManager") as MockDBM:
            mock_mgr = MagicMock()
            mock_mgr.list_devboxes.return_value = [
                {"name": "shraga-box-01", "provisioningState": "Succeeded", "powerState": "Running"},
                {"name": "shraga-box-02", "provisioningState": "Succeeded", "powerState": "Stopped"},
            ]
            MockDBM.return_value = mock_mgr

            exit_code = cds_main(["--user", "aad-user-abc"])
    ...
    def test_check_devbox_status_cli_not_found(self, devbox_env_vars, capsys):
        from check_devbox_status import main as cds_main

        with patch("check_devbox_status.DevBoxManager") as MockDBM:
            mock_mgr = MagicMock()
            mock_mgr.get_devbox_status.side_effect = Exception(
                "Failed to get Dev Box status: 404 Not Found"
            )
            MockDBM.return_value = mock_mgr

            exit_code = cds_main(["--name", "nonexistent-box", "--user", "aad-user-abc"])
    ...
    def test_check_devbox_status_cli_missing_env_vars(self, monkeypatch, capsys):
        from check_devbox_status import main as cds_main

        monkeypatch.delenv("DEVCENTER_ENDPOINT", raising=False)
        monkeypatch.delenv("DEVBOX_PROJECT", raising=False)

        with pytest.raises(SystemExit) as exc_info:
            cds_main(["--name", "shraga-box-01", "--user", "aad-user-abc"])

        assert exc_info.value.code == 1

    def test_check_devbox_status_cli_user_required(self):
        from check_devbox_status import main as cds_main

        with pytest.raises(SystemExit) as exc_info:
            cds_main(["--name", "shraga-box-01"])

        assert exc_info.value.code == 2

    def test_check_devbox_status_cli_json_output(self, devbox_env_vars, capsys):
        from check_devbox_status import main as cds_main

        with patch("check_devbox_status.DevBoxManager") as MockDBM:
            mock_mgr = MagicMock()
            mock_mgr.get_devbox_status.side_effect = Exception("Server error 500")
            MockDBM.return_value = mock_mgr

            exit_code = cds_main(["--name", "box", "--user", "user"])
...
class TestCheckDevboxBuildParser:
    """Test argument parser configuration."""

    def test_name_is_optional(self):
        from check_devbox_status import build_parser as cds_build_parser
        parser = cds_build_parser()
        args = parser.parse_args(["--user", "abc123"])
        assert args.name is None
        assert args.user == "abc123"

    def test_name_and_user(self):
        from check_devbox_status import build_parser as cds_build_parser
        parser = cds_build_parser()
        args = parser.parse_args(["--name", "shraga-box-01", "--user", "abc123"])
        assert args.name == "shraga-box-01"
        assert args.user == "abc123"
    ...
    def test_user_is_required(self):
        ...
        with pytest.raises(SystemExit):
            parser.parse_args(["--name", "shraga-box-01"])
